...
bool AppleACPIPS2Nub::start(IOService *provider)
{
    ...
    if(m_mouseProvider != NULL)
    {
        ...
        if(attach(m_mouseProvider))
        {
            ...
            if(m_interruptControllers && m_interruptControllers->getCount() != 0 &&
               m_interruptSpecifiers && m_interruptSpecifiers->getCount() != 0)
            {
...
IOReturn AppleACPIPS2Nub::message( UInt32 type, IOService * provider, void * argument)
{
    ...
    messageClients(kIOACPIMessageDeviceNotification, argument);
    return kIOReturnSuccess;
}