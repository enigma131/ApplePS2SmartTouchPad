...
// Warning FIX: Remove deprecated `register` keyword
...
bool ApplePS2Controller::init(OSDictionary * properties)
{
    ...
    _muxModeRoutePrefix = kRP_RoutingPrefixAuxPort;
    _controllerUsesMuxMode = false;
...
bool ApplePS2Controller::start(IOService * provider)
{
    ...
    IOLog("STD :: Enhanced Apple PS2 Controller and Keyboard driver v4.7 Copyright (c) EMlyDinEsH <www.osxlatitude.com> 2012-2016.\n");
    ...
    for (int index = 0; index < kKeyboardQueueSize; index++)
        queue_enter(&_keyboardQueueUnused, &_keyboardQueueAlloc[index], KeyboardQueueElement *, chain);
#endif
    ...
    //  touchpad, e.g., some Gigabyte laptop models with ELAN
    //  touchpads). Assuming it won't break other systems so applied
    ...
    res = readDataPort(kDT_Keyboard);
    if (res != kSC_Acknowledge) {
        IOLog("STD :: Failed to do the Keyboard reset, response received 0x%x\n", res);
    }
    
    //
    // Check for the active multiplexing mode support in the controller
    // and enable the support if it suppports.
    //
    
    detectMuxModeSupport();
...
void ApplePS2Controller::stop(IOService * provider)
{
    ...
    assert(_fnKeysnotificationInstalledKeyboard == false);
    assert(_fnKeynotifiationInstalledMouseTouchpad == false);
    ...
    // Free the work loop.
    RELEASE(_workLoop);
    // Free the command gate.
    RELEASE(_commandGate);
...
void ApplePS2Controller::resetPS2Controller()
{
    DEBUG_LOG("STD :: Resetting PS2 Controller ...\n");

    UInt8 commandByte;
    
    //
    // Reset and Initialize the mouse and keyboard hardware to a known state --
    // the IRQs are disabled (don't want interrupts), the clock line is enabled (want to
    ...
    // Clear out garbage in the controller's input streams.
...
void ApplePS2Controller::detectMuxModeSupport()
{
    
    //
    //  Check for the mulitplexing support.
    //
    
    _controllerUsesMuxMode = false;
    
    if (!setMuxMode(true)) {
        DEBUG_LOG("STD :: Controller doesn't support active multiplexing, using legacy mode.\n");
    }
    else {
        ...
        commandByte |=  kCB_DisableMouseClock;
        commandByte &= ~kCB_EnableMouseIRQ;
        writeCommandPort( kCP_SetCommandByte );
        writeDataPort( commandByte );
        
        _controllerUsesMuxMode = true;
    }
...
{
    //
    //  Sending multiplexing mode activation sequence which is
    ...
    writeDataPort(val);
    res = readDataPort(kDT_Mouse);
    
    DEBUG_LOG("STD :: Response for the Mux activation sequence one Data (0x%x) 0x%x\n", val, res);
    ...
    DEBUG_LOG("STD :: Response for the Mux activation sequence two Data (0x%x) 0x%x\n", val, res);
    ...
    DEBUG_LOG("STD :: Response for the Mux activation sequence three Data (0x%x) 0x%x\n", val, res);
    
    if (res == val) {
        DEBUG_LOG("STD :: Not using active multiplexing controller.\n");
        return false;
    }
    
    DEBUG_LOG("STD :: Detected active multiplexing controller, rev %d.%d\n", (res >> 4) & 0xf, res & 0xf);
    
    //
    //  Find the Aux port of the touchpad by sending the custom command (0xF8)
    //  for which it wil acknowledge (0xFA) if its an touchpad, otherwise it will reject (0xFE).
    //  Mostly Port 1 is unused and its bound to regular mouse, other ports check done here
    //  just to find the status.
    //
    
    bool foundPort = false;
    DEBUG_LOG("STD :: Detecting the Mouse Aux port used by the touchpad...\n");
    
    for (int i = 0; i < 4; i++) {
        
        writeCommandPort(kRP_RoutingPrefixAuxPort + i);
        writeDataPort(0xf8);
        res = readDataPort(kDT_Mouse);
        
        DEBUG_LOG("STD :: Aux#%d port response for the command %x.\n", i, res);
        
        if (res == 0xfa) {
            foundPort = true;
            _muxModeRoutePrefix = kRP_RoutingPrefixAuxPort + i;
            DEBUG_LOG("STD :: Aux#%d port is used by the touchpad.\n", i);
            break;
        }
    }
    
    if (!foundPort) {
        DEBUG_LOG("STD :: Failed to find tha mouse aux port.\n");
        return false;
    }
    else
        return true;
...
void ApplePS2Controller::submitRequestAndBlock(PS2Request * request)
{
    ...
    //
    _commandGate->runAction(OSMemberFunctionCast(IOCommandGate::Action, this, &ApplePS2Controller::submitRequestAndBlockGated), request);
}
...
{                                                      // PS2CompletionAction
    // does nothing, kept this only for any future uses
}
...
{                                                      // IOInterruptEventAction
    ...
    while (1)
    {
        ...
        else if (((status = inb(kCommandPort)) & (kOutputReady | kMouseData)) == (kOutputReady | kMouseData))
        {
            unlockController(state);
    ...
    while ( ((status = inb(kCommandPort)) & kOutputReady) )
    {
        ...
        dispatchDriverInterrupt((status&kMouseData)?kDT_Mouse:kDT_Keyboard, inb(kDataPort));
    }
#endif
}
    ...
    for (index = 0; index < request->commandsCount; index++)
    {
        switch (request->commands[index].command)
        {
            ...
            case kPS2C_ReadMouseDataPort:
                ...
                request->commands[index].inOrOut = readDataPort(kDT_Mouse);
                break;
            ...
            case kPS2C_WriteCommandPort:
                ...
                //    kPS2C_WriteCommandPort ( kRP_RoutingPrefixAuxPort# ) (multiplex mode)
            ...
            case kPS2C_SendMouseCommandAndCompareAck:
                ...
                if (_controllerUsesMuxMode)
                    writeCommandPort(_muxModeRoutePrefix);
                else
                    writeCommandPort(kCP_TransmitToMouse);
                ...
                DEBUG_LOG("STD :: Mouse Data %x, ACK %x, Indx %d, Controller uses Mux %s, Mux Route Prefix 0x%x\n", request->commands[index].inOrOut, byte, index, _controllerUsesMuxMode?"Yes, ":"No", _muxModeRoutePrefix);
...
UInt8 ApplePS2Controller::readDataPort(PS2DeviceType deviceType)
{
    ...
    UInt8  readByte;
    UInt8  status = 0;
    UInt32 timeoutCounter = 10000;    // (timeoutCounter * kDataDelay = 200 ms)
    ...
    while (1)
    {
        ...
        if (timeoutCounter == 0)
        {
            ...
            DEBUG_LOG("STD :: Timed out on %s input stream with status 0x%x\n", (deviceType == kDT_Keyboard) ? "keyboard" : "mouse", status);
                                       ...
                                       UInt8         expectedByte)
{
    ...
    UInt8  firstByte     = 0, secondByte = 0, thirdByte = 0;
    bool   firstByteHeld = false, secondByteHeld = false, thirdByetHeld = false;
    UInt8  readByte;
    ...
    while (1)
    {
        ...
        if (timeoutCounter == 0)
        {
            ...
            DEBUG_LOG("STD :: Timed out on %s input stream.\n", (deviceType == kDT_Keyboard) ? "keyboard" : "mouse");
        ...
        if (requestedStream)
        {
            if (readByte == expectedByte)
            {
                ...
                else
                {
                    ...
                    // Dispatch the previous byte(s) to the interrupt handler, and return the matched.
                    //
                    
                    dispatchDriverInterrupt(deviceType, firstByte);
                    
                    if (secondByteHeld)
                        dispatchDriverInterrupt(deviceType, secondByte);
                    
                    if (thirdByetHeld)
                        dispatchDriverInterrupt(deviceType, thirdByte);
            ...
            else // (readByte does not match expectedByte)
            {
                if (firstByteHeld == false)
                {
                    ...
                    firstByteHeld = true;
                    firstByte     = readByte;
                    DEBUG_LOG("STD :: First byte %x not expected\n", readByte);

                }
                else if (secondByteHeld == false)
                {
                    //
                    // The second byte was received, and does not match the byte we are
                    // expecting.  Put it aside for the moment.
                    // I [EMlyDinEsH] have seen this case in Dell [ELAN] when sending reset
                    // on system warm boots.
                    //
                    
                    secondByteHeld = true;
                    secondByte     = readByte;
                    DEBUG_LOG("STD :: Second byte %x not expected\n", readByte);
                }
                else if (thirdByetHeld == false)
                {
                    //
                    // The third byte was received, and does not match the byte we are
                    // expecting.  Put it aside for the moment.
                    // I [EMlyDinEsH] have seen this case in Lenvo [Synaptics] when sending reset
                    // after system sleep.
                    //
                    
                    thirdByetHeld = true;
                    thirdByte     = readByte;
                    DEBUG_LOG("STD :: Third byte %x not expected\n", readByte);
                }
                else if (readByte != expectedByte)
                {
                    //
                    // The third byte mismatched as well.
                    // Mostly this wont occur, so no error logged.
                    //
    ...
    if ( _currentPowerState != powerState )
    {
        switch ( powerState )
        {
            ...
            case kPS2PowerStateNormal:
                ...
                //
                // Re-Detect active multiplexing mode support in the controller
                // and enable if it suppports (Optional)
                //
            ...
            default:
                DEBUG_LOG("STD :: ApplePS2Controller received bad power state %d\n", powerState);
                break;
        }
...
IOReturn ApplePS2Controller::message( UInt32 type, IOService * provider, void * argument)
{
    DEBUG_LOG("%s: ACPI Message, Type %x Provider %s, Event %d \n", this->getName(), uint(type), provider->getName(), *((UInt32 *) argument));
    
    UInt32 eventCode = *((UInt32 *) argument);
    if (eventCode > 0)
    {
        switch (eventCode) {
            case 0x10: // brightness up
                eventCode = kPS2C_BrightnessUp;
                break;
                
            case 0x20: // brightness down
                eventCode = kPS2C_BrightnessDown;
                break;
        }
        
        if (_ps2notificationInstalledKeyboard)
            (*_ps2notificationActionKeyboard)(_ps2notificationTargetKeyboard, eventCode);
    }
